...
async def init_db():
    client = None
    try:
        # 1. Attempt connection to primary MONGODB_URI
        ...
        logger.warning(f"⚠️ Could not connect to external MongoDB at {settings.MONGODB_URI}: {e}")
        logger.info("Starting in-memory MongoMock Motor fallback...")
        try:
            import mongomock
            import mongomock_motor
            
            # Patch list_collection_names for mongomock compatibility with beanie
            orig_list_cols = mongomock.Database.list_collection_names
            def patched_list_cols(self, *args, **kwargs):
                return orig_list_cols(self)
            mongomock.Database.list_collection_names = patched_list_cols

            client = mongomock_motor.AsyncMongoMockClient()
            logger.info("✓ Connected to In-Memory AsyncMongoMockClient")
        except Exception as mock_err:
            logger.error(f"❌ Failed to start mock MongoDB: {mock_err}")
            raise e

    # 2. Initialize Beanie with document models
    db_name = settings.MONGODB_URI.split("/")[-1].split("?")[0] or "codingcon"
    await init_beanie(
    ...
    # 3. Auto-seed initial demo data if empty
...
async def ensure_seed_data():
    ...
    logger.info("── Auto-seeding initial database collections for FastAPI …")
    ...
    logger.info("✓ Initial seed completed successfully for FastAPI")
